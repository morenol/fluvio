...
pub struct VehiclePosition {
    ...
    pub long: f32,          // WGS 84 longitude in degrees.null if location is unavailable.
    ...
    pub spd: f32,           // Speed of the vehicle, in meters per second (m/s).
    pub hdg: u16, // Heading of the vehicle, in degrees (⁰) starting clockwise from geographic north. Valid values are on the closed interval [0, 360].
    pub acc: f32, // Acceleration (m/s^2), calculated from the speed on this and the previous message. Negative values indicate that the speed of the vehicle is decreasing.
    pub dl: u16, // Offset from the scheduled timetable in seconds (s). Negative values indicate lagging behind the schedule, positive values running ahead of schedule.
    pub odo: u32, // The odometer reading in meters (m) since the start of the trip. Currently the values not very reliable.
    pub drst: u32, // Door status. 0 if all the doors are closed. 1 if any of the doors are open.
    ...
    pub jrn: u32,     // Internal journey descriptor, not meant to be useful for external use.
    pub line: u16,    // Internal line descriptor, not meant to be useful for external use.
    ...
    pub stop: u32, // ID of the stop related to the event (e.g. ID of the stop where the vehicle departed from in case of dep event or the stop where the vehicle currently is in case of vp event).null if the event is not related to any stop.
